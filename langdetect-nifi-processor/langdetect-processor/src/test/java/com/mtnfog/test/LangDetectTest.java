...
public class LangDetectTest {
    ...
    public void detectEnglish() throws IOException {
        ...
        final String json = IOUtils.toString(runner.getContentAsByteArray(result), "UTF-8");
        
        System.out.println(json);
        
        Gson gson = new Gson();
        Type listType = new TypeToken<List<Language>>(){}.getType();
        List<Language> languages = gson.fromJson(json, listType);
        
        assertTrue(languages.size() > 1);
        assertEquals("eng", languages.get(0).getLang());
